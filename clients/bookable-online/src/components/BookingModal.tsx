      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-6 border-b border-gray-100">
          <div className="flex items-center gap-3">
            <Calendar className="w-6 h-6 text-bookable-green" />
        ...
        <div className="p-6">
          ...
          {step === 'loading' && (
            <div className="text-center py-12">
              <Loader2 className="w-10 h-10 text-bookable-green animate-spin mx-auto mb-4" />
          ...
          {step === 'error' && (
            <div className="text-center py-12">
              ...
              <button
                ...
                className="bg-bookable-green text-white font-medium px-6 py-3 rounded-lg hover:bg-green-700 transition-colors"
              >
                Try again
              </button>
            </div>
          ...
          {step === 'confirmed' && (
            <div className="text-center py-8">
              <CheckCircle className="w-16 h-16 text-bookable-green mx-auto mb-4" />
              ...
              <button
                onClick={handleClose}
                className="mt-6 bg-bookable-green text-white font-medium px-6 py-3 rounded-lg hover:bg-green-700 transition-colors"
              >
                Done
              </button>
            </div>
                          ...
                          <button
                            ...
                            className={`px-4 py-2 rounded-lg text-sm font-medium border transition-all ${
                              selectedDate === slot.date && selectedTime === time
                                ? 'bg-bookable-green text-white border-bookable-green'
                                : 'border-gray-200 text-gray-700 hover:border-bookable-green hover:text-bookable-green'
                            }`}
                          >
                            {time}
                          </button>
          ...
          {step === 'details' && (
            <form onSubmit={handleBook} className="space-y-4">
              <div className="bg-bookable-green/5 rounded-lg p-3 mb-4">
                <p className="text-sm text-bookable-green font-medium">
                  {slots.find((s) => s.date === selectedDate)?.display} at {selectedTime}
                  <button
                    type="button"
                    onClick={() => setStep('date')}
                    className="ml-2 underline text-bookable-green/70 hover:text-bookable-green"
                  >
                    Change
                  </button>
                </p>
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full px-4 py-3 rounded-lg border border-gray-200 focus:border-bookable-green focus:ring-2 focus:ring-bookable-green/20 outline-none transition-all resize-none"
              ...
              <button
                type="submit"
                disabled={sending}
                className="w-full bg-bookable-green text-white font-semibold px-8 py-4 rounded-lg text-lg hover:bg-green-700 transition-colors disabled:opacity-60"
              >
