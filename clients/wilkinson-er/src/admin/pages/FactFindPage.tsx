 ...
 *
 * The 21 section components live in src/admin/factfind/sections/ and are
...
function CasePicker() {
  ...
  useEffect(() => {
    supabase
      ...
      .then(({ data }) => {
        setCases((data as unknown as typeof cases) ?? [])
        setLoading(false)
      })
  }, [])
...
function FactFindForm({ caseId }: { caseId: string }) {
  ...
  useEffect(() => {
    supabase
      ...
      .then(({ data }) => {
        if (data) {
          const contact = data.contact as unknown as { first_name: string; last_name: string } | null
