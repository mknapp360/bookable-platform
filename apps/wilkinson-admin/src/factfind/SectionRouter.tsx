/**
 * SectionRouter — renders the active fact-find section.
 *
 * SETUP: Copy the 21 section components from
 *   er-site/src/admin/factfind/sections/
 * into this folder:
 *   apps/wilkinson-admin/src/factfind/sections/
 *
 * Then uncomment each import below and replace the corresponding
 * placeholder with the real component.
 *
 * The section components are pure (data + onChange props only) —
 * they don't need any changes beyond moving the file.
 */
...
function Placeholder({ section }: { section: string }) {
  return (
    <div className="bg-white rounded-xl border border-dashed border-slate-200 p-8 text-center text-sm text-slate-400">
      <p className="font-medium text-slate-500 mb-1">{section}</p>
      <p>Copy the section component from er-site into src/factfind/sections/ and uncomment the import in SectionRouter.tsx</p>
    </div>
  )
}
...
export function SectionRouter({ activeSection, data, onChange, factFind }: Props) {
  ...
  // Suppress unused variable warnings until sections are wired in
  void mortgagesData; void unsecuredDebtsData; void personalDetailsData; void clientDetailsData

  switch (activeSection) {
    // Replace each Placeholder with the real component as you copy sections in:
