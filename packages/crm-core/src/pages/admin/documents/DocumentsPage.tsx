...
export function DocumentsPage() {
  ...
  const { documents, loading, uploadDocument, deleteDocument, getSignedUrl } = useDocuments(tenantId)
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-0.5">
            {documents.length} {documents.length === 1 ? 'document' : 'documents'} in {tenant?.name}
          </p>
        </div>
        <button
